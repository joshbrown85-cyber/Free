...
// Recolored to sit alongside the current true-black/terracotta theme:
// muted, low-saturation tones rather than the original build's brighter set,
// since terracotta now carries the only strongly saturated color on screen.
const COLORS = [
  {hex:'#7c8f86', name:'sage'},
  {hex:'#d9a86c', name:'amber'},
  {hex:'#9a8fc4', name:'violet'},
  {hex:'#8fb3d9', name:'blue'},
  {hex:'#c48f8f', name:'rose'},
  {hex:'#6f9c94', name:'teal'},
];
...
// One-time downgrade from a newer build's data shape (the "Check-in" /
// "Ledger" redesigns, which stored trackers as {start, runs, order, ...}
// instead of this build's {startedAt}, plus separate checkins/notes/triggers
// keys instead of journal/streakHistory). Runs once on first load after a
// revert so real tracked progress carries over instead of looking empty.
async function downgradeFromNewerSchema(){
  try{
    if(await window.storage.get('downgradedFromNewerSchema')) return;
    const newTrackers = await window.storage.get('trackers');
    const looksNewer = Array.isArray(newTrackers) && newTrackers.some(t => t && t.start && !t.startedAt);
    if(!looksNewer){
      await window.storage.set('downgradedFromNewerSchema', true);
      return;
    }

    const newReasons = (await window.storage.get('reasons')) || [];
    const newNotes = (await window.storage.get('notes')) || [];
    const newTriggers = (await window.storage.get('triggers')) || [];
    const newSearches = (await window.storage.get('searches')) || [];
    const triggerLabel = id => { const v = newTriggers.find(x => x.id === id); return v ? v.label : ''; };

    const oldTrackers = newTrackers.map(t => ({
      id: t.id, name: t.name, color: t.color, startedAt: t.start, hidden: !!t.hidden
    }));

    const oldHistory = [];
    newTrackers.forEach(t => {
      (t.runs || []).forEach(r => {
        const endedAt = r.endedOn;
        const startedAt = new Date(new Date(endedAt).getTime() - (r.length || 0) * 86400000).toISOString();
        oldHistory.push({ id: uid(), trackerId: t.id, startedAt, endedAt });
      });
    });

    const oldReasons = newReasons.map(r => ({
      id: r.id, trackerId: r.trackerId, text: r.text, createdAt: r.writtenAt || new Date().toISOString()
    }));

    const oldJournal = newNotes.map(n => ({
      id: n.id, trackerId: n.trackerId, trigger: triggerLabel(n.triggerId), text: n.text, createdAt: n.createdAt
    }));

    const oldCustomTopics = [];
    newSearches.forEach(s => {
      const k = (s.query || '').toLowerCase().replace(/[^a-z0-9 ]/g, '').replace(/\s+/g, '-');
      if(k && !oldCustomTopics.some(c => c.k === k)){
        oldCustomTopics.push({ k, l: s.query.charAt(0).toUpperCase() + s.query.slice(1) });
      }
    });

    await window.storage.set('trackers', oldTrackers);
    await window.storage.set('streakHistory', oldHistory);
    await window.storage.set('reasons', oldReasons);
    await window.storage.set('journal', oldJournal);
    await window.storage.set('customTopics', oldCustomTopics);
    await window.storage.set('downgradedFromNewerSchema', true);
  }catch(e){ console.error('downgrade from newer schema failed', e); }
}

async function loadState(){
  await downgradeFromNewerSchema();
  try{
